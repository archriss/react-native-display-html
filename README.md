...
> In addition to these props, you can provide **any prop from the [Webview component](https://facebook.github.io/react-native/docs/webview.html)** excepted `injectedJavaScript`, `source` and `onMessage` which are handled in a specific way.
...
Instead of sending plain strings as in React Native's core Webview component, you can now directly send functions. Just make an array of all the functions you need to inject, and react-native-display-html will handle stringify-ing them and adding closure, ez pz.

Using an array might allow you to inject javascript only on some devices that might need it.

### Sending data from webview to react-native

You can send data by using `window.WebViewBridge.send` from the webview's javascript. You can use it in your injected code. For a demo of how it's working, check out the `heightScript` method of this component.
...
Then, in your `HTMLStyles` props, you can declare a font with `url("/pathtoyourfont.ttf")` for iOS and `url("file:///android_asset/pathtoyourfont.ttf")` on Android.
Be sure to have your fonts inside your XCode and Android projects.
